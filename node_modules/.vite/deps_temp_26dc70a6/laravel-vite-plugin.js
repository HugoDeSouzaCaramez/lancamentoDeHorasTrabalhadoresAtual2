import "./chunk-DLB6AKCE.js";
import {
  loadEnv,
  normalizePath$3
} from "./chunk-KZX3SYJF.js";
import {
  require_os
} from "./chunk-R3VMYURF.js";
import "./chunk-PJUNNFNB.js";
import "./chunk-HO7OSK64.js";
import "./chunk-3NWPCB5S.js";
import "./chunk-UQC37NDZ.js";
import "./chunk-JQENMGEB.js";
import {
  require_picocolors_browser
} from "./chunk-FRZFHJ4B.js";
import {
  require_url
} from "./chunk-PKGPE476.js";
import {
  require_fs
} from "./chunk-F73QYG3F.js";
import {
  require_path
} from "./chunk-LTFDKGDS.js";
import {
  __commonJS,
  __toESM
...
// node_modules/vite-plugin-full-reload/dist/index.js
...
function normalizePaths(root, path2) {
  return (Array.isArray(path2) ? path2 : [path2]).map((path22) => (0, import_path.resolve)(root, path22)).map(normalizePath$3);
}
  ...
  configureServer({ watcher, ws, config: { logger } }) {
    ...
    const checkReload = (path2) => {
      if (shouldReload(path2)) {
        setTimeout(() => ws.send({ type: "full-reload", path: always ? "*" : path2 }), delay);
        if (log)
          logger.info(`${import_picocolors.default.green("page reload")} ${import_picocolors.default.dim((0, import_path.relative)(root, path2))}`, { clear: true, timestamp: true });
      }
    };
    watcher.add(files);
...
function resolveLaravelPlugin(pluginConfig) {
  ...
  return {
    ...
    config: (config, { command, mode }) => {
      ...
      userConfig = config;
      const ssr = !!((_a = userConfig.build) == null ? void 0 : _a.ssr);
      ...
      return {
        ...
        build: {
          manifest: ((_b = userConfig.build) == null ? void 0 : _b.manifest) ?? (ssr ? false : "manifest.json"),
          ssrManifest: ((_c = userConfig.build) == null ? void 0 : _c.ssrManifest) ?? (ssr ? "ssr-manifest.json" : false),
          outDir: ((_d = userConfig.build) == null ? void 0 : _d.outDir) ?? resolveOutDir(pluginConfig, ssr),
          rollupOptions: {
            input: ((_f = (_e = userConfig.build) == null ? void 0 : _e.rollupOptions) == null ? void 0 : _f.input) ?? resolveInput(pluginConfig, ssr)
          },
          assetsInlineLimit: ((_g = userConfig.build) == null ? void 0 : _g.assetsInlineLimit) ?? 0
        },
        server: {
          origin: ((_h = userConfig.server) == null ? void 0 : _h.origin) ?? "__laravel_vite_placeholder__",
          ...process.env.LARAVEL_SAIL ? {
            host: ((_i = userConfig.server) == null ? void 0 : _i.host) ?? "0.0.0.0",
            port: ((_j = userConfig.server) == null ? void 0 : _j.port) ?? (env.VITE_PORT ? parseInt(env.VITE_PORT) : 5173),
            strictPort: ((_k = userConfig.server) == null ? void 0 : _k.strictPort) ?? true
          } : void 0,
          ...serverConfig ? {
            host: ((_l = userConfig.server) == null ? void 0 : _l.host) ?? serverConfig.host,
            hmr: ((_m = userConfig.server) == null ? void 0 : _m.hmr) === false ? false : {
              ...serverConfig.hmr,
              ...((_n = userConfig.server) == null ? void 0 : _n.hmr) === true ? {} : (_o = userConfig.server) == null ? void 0 : _o.hmr
            },
            https: ((_p = userConfig.server) == null ? void 0 : _p.https) ?? serverConfig.https
          } : void 0
        },
        resolve: {
          alias: Array.isArray((_q = userConfig.resolve) == null ? void 0 : _q.alias) ? [
            ...((_r = userConfig.resolve) == null ? void 0 : _r.alias) ?? [],
            ...
            ...defaultAliases,
            ...(_s = userConfig.resolve) == null ? void 0 : _s.alias
          }
        },
        ssr: {
    ...
    configureServer(server) {
      ...
      (_a = server.httpServer) == null ? void 0 : _a.once("listening", () => {
        ...
        if (isAddressInfo(address)) {
          viteDevServerUrl = ((_b = userConfig.server) == null ? void 0 : _b.origin) ? userConfig.server.origin : resolveDevServerUrl(address, server.config, userConfig);
          import_fs.default.writeFileSync(pluginConfig.hotFile, viteDevServerUrl);
          setTimeout(() => {
            server.config.logger.info(`
  ${import_picocolors2.default.red(`${import_picocolors2.default.bold("LARAVEL")} ${laravelVersion()}`)}  ${import_picocolors2.default.dim("plugin")} ${import_picocolors2.default.bold(`v${pluginVersion()}`)}`);
            server.config.logger.info("");
            server.config.logger.info(`  ${import_picocolors2.default.green("➜")}  ${import_picocolors2.default.bold("APP_URL")}: ${import_picocolors2.default.cyan(appUrl.replace(/:(\d+)/, (_, port) => `:${import_picocolors2.default.bold(port)}`))}`);
            if (typeof resolvedConfig.server.https === "object" && typeof resolvedConfig.server.https.key === "string") {
              if (resolvedConfig.server.https.key.startsWith(herdConfigPath())) {
                server.config.logger.info(`  ${import_picocolors2.default.green("➜")}  Using Herd certificate to secure Vite.`);
              }
              if (resolvedConfig.server.https.key.startsWith(valetConfigPath())) {
                server.config.logger.info(`  ${import_picocolors2.default.green("➜")}  Using Valet certificate to secure Vite.`);
              }
            }
          }, 100);
        }
      ...
      return () => server.middlewares.use((req, res, next) => {
        if (req.url === "/index.html") {
          res.statusCode = 404;
          res.end(
            import_fs.default.readFileSync(import_path2.default.join(dirname(), "dev-server-index.html")).toString().replace(/{{ APP_URL }}/g, appUrl)
          );
        }
        next();
      });
...
function laravelVersion() {
  ...
  try {
    ...
    return ((_b = (_a = composer.packages) == null ? void 0 : _a.find((composerPackage) => composerPackage.name === "laravel/framework")) == null ? void 0 : _b.version) ?? "";
  } catch {
    return "";
  }
}
function pluginVersion() {
  var _a;
  try {
    return (_a = JSON.parse(import_fs.default.readFileSync(import_path2.default.join(dirname(), "../package.json")).toString())) == null ? void 0 : _a.version;
  } catch {
    return "";
  }
}
function resolvePluginConfig(config) {
  ...
  return {
    ...
    hotFile: config.hotFile ?? import_path2.default.join(config.publicDirectory ?? "public", "hot"),
...
function resolveOutDir(config, ssr) {
  ...
  return import_path2.default.join(config.publicDirectory, config.buildDirectory);
}
...
function resolveDevServerUrl(address, config, userConfig) {
  ...
  const sailHost = process.env.LARAVEL_SAIL && !((_a = userConfig.server) == null ? void 0 : _a.host) ? "localhost" : null;
...
function noExternalInertiaHelpers(config) {
  var _a;
  const userNoExternal = (_a = config.ssr) == null ? void 0 : _a.noExternal;
...
function resolveDevelopmentEnvironmentServerConfig(host) {
  ...
  const resolvedHost = host === true || host === null ? import_path2.default.basename(process.cwd()) + "." + resolveDevelopmentEnvironmentTld(configPath) : host;
  const keyPath = import_path2.default.resolve(configPath, "Certificates", `${resolvedHost}.key`);
  const certPath = import_path2.default.resolve(configPath, "Certificates", `${resolvedHost}.crt`);
...
function determineDevelopmentEnvironmentConfigPath() {
  if (import_fs.default.existsSync(herdConfigPath())) {
    return herdConfigPath();
  }
  if (import_fs.default.existsSync(valetConfigPath())) {
    return valetConfigPath();
  }
}
function resolveDevelopmentEnvironmentTld(configPath) {
  const configFile = import_path2.default.resolve(configPath, "config.json");
...
function dirname() {
  return (0, import_url.fileURLToPath)(new URL(".", import.meta.url));
}
function herdConfigPath() {
  return import_path2.default.resolve(import_os.default.homedir(), "Library", "Application Support", "Herd", "config", "valet");
}
function valetConfigPath() {
  return import_path2.default.resolve(import_os.default.homedir(), ".config", "valet");
}
export {
  laravel as default,
  refreshPaths
